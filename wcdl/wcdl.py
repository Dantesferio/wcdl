...
def main():
    parser = argparse.ArgumentParser(
        "wcdl", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument(
        "-s", "--search",
        action="store",
        default="",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["all"],
        help="""specifing the chapter to be selected
    ...
    if args.local_search:
        ...
        if type(search_result) is int:
            error(
                f"connection error while searching online, code:{search_result}")
            exit(1)
        if search_result == []:
        ...
        table = rich.table.Table(
            title=f"results : {search_prompt}", box=rich.box.ROUNDED, show_lines=True)
        ...
        for result in search_result:
            ...
            n += 1
        rich.print(table)
        selected = rich.prompt.Prompt.ask("choice to download", choices=[
                                          str(i) for i in range(0, n)], default=1, show_choices=False)
    ...
    if args.download:
        notic(
            f"downloading selected range of chapters --> {' '.join(args.range)}")

        downloader.download_chapters_progress(
            selected_manga["name"], selected_chapters)
        success("Done")
    if args.save_to_json:
        downloader.save_data_to_json(selected_manga["name"], selected_chapters)
        success(
            f"manga data has been saved to {selected_manga["name"]}.json file")
